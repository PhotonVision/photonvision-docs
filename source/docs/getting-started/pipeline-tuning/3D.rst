...
In 3D mode, the SolvePNP algorithm is used to compute the position and rotation of the target relative to the robot. This requires your camera to be calibrated :ref:`camera to be calibrated <docs/getting-started/pipeline-tuning/calibration>`, which can be done through the cameras tab.
